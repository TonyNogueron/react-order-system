...
function EditOrder({initialState}) {
    ...
    const setValues = () => {
        ...
        setFolio(initialState.folio);
        setNombre(initialState.nombreCompleto);
        setHora(initialState.hora);
        setNumero(initialState.telefono);
        setCantidadPaquetes(initialState.paquete);
        setPagado(initialState.pagado);
        setEntregado(initialState.entregado);
        setRelleno(initialState.polloRelleno);
        setAdobado(initialState.polloAdobado);
        setNatural(initialState.polloNatural);
        setLomo(initialState.lomo);
        setPierna(initialState.pierna);
        setCostilla(initialState.costilla);
        setManzana(initialState.manzana);
        setCoditos(initialState.codo);
        setSpaguetti(initialState.espaguetti);
        setRusa(initialState.rusa);
        setChiles(initialState.chiles);
        setOtros(initialState.otros);
        setAnticipo(initialState.anticipo);
        setRestante(initialState.resta);
        setTotal(initialState.total);
    }

    const updateOrder = (e) => {
        e.preventDefault();
        axios.put("http://localhost:3001/editarOrden", pedidoJson, { params: { id: id } })
        .then((res) => {
            console.log(res);
  ...
  return (
     <div className="Form">
            ...
            <section className="top-bottom">
                ...
                <div className="BotBottom">
                    ...
                    <div className="Pagado">
                        <h1>Pagado</h1>
                        <div className = "wrapper">
                            <input type="checkbox" className="num" checked={Pagado} value={Pagado} onChange={e => setPagado(e.target.checked)}/>
                        </div>
                    </div>
                    <div className="Entregado">
                        <h1>Entregado</h1>
                        <div className = "wrapper">
                            <input type="checkbox" className="num" checked={Entregado} value={Entregado} onChange={e => setEntregado(e.target.checked)}/>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <button className="Enviar" onClick={updateOrder}>Actualizar</button>
        </div>
  );
}
